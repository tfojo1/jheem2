...
create.style.manager <- function(name,
                                 description,
                                 aes.for.simset = 'linetype',
                                 aes.for.split.by = 'color',
                                 aes.for.simset.single.point = 'shape',
                                 aes.for.location = 'shape',
                                 name.for.sim.color.legend = 'sim color',
                                 name.for.data.color.legend = 'data color',
                                 name.for.data.shape.legend = 'shape',
                                 alpha.ribbon = 0.2,
                                 set.as.default = F)
{
    new.style.manager = JHEEM.STYLE.MANAGER$new(name, 
                                                description=description,
                                                aes.for.simset = aes.for.simset,
                                                aes.for.split.by = aes.for.split.by,
                                                aes.for.simset.single.point = aes.for.simset.single.point,
                                                aes.for.location = aes.for.location,
                                                name.for.sim.color.legend = name.for.sim.color.legend,
                                                name.for.data.color.legend = name.for.data.color.legend,
                                                name.for.data.shape.legend = name.for.data.shape.legend,
...
JHEEM.STYLE.MANAGER = R6::R6Class(
    ...
    public = list(
                              ...
                              alpha.ribbon = alpha.ribbon)
        {
            ...
            private$i.aes.for.simset = aes.for.simset
            private$i.aes.for.split.by = aes.for.split.by
            private$i.aes.for.simset.single.point = aes.for.simset.single.point
            private$i.aes.for.location = aes.for.location
            private$i.name.for.sim.color.legend = name.for.sim.color.legend
            private$i.name.for.data.color.legend = name.for.data.color.legend
            private$i.name.for.data.shape.legend = name.for.data.shape.legend
    ...
    active = list(
        ...
        name.for.sim.color.legend = function(value)
        {
            if (missing(value))
                private$i.name.for.sim.color.legend
            else
                stop("Cannot modify 'name.for.sim.color.legend' in jheem.data.manager - it is read-only")
        },
        
        name.for.data.color.legend = function(value)
        {
            if (missing(value))
                private$i.name.for.data.color.legend
            else
                stop("Cannot modify 'name.for.data.color.legend' in jheem.data.manager - it is read-only")
        },
        
        name.for.data.shape.legend = function(value)
        {
            if (missing(value))
                private$i.name.for.data.shape.legend
            else
                stop("Cannot modify 'name.for.data.shape.legend' in jheem.data.manager - it is read-only")
        },
        ...
        alpha.ribbon = function(value)
        {
            if (missing(value))
                ...
                stop("Cannot modify 'alpha.ribbon' in jheem.data.manager - it is read-only")
        }
    ),
    ...
    private = list(
        ...
        i.name.for.sim.color.legend = NULL,
        i.name.for.data.color.legend = NULL,
        i.name.for.data.shape.legend = NULL,
        i.alpha.ribbon = NULL,
