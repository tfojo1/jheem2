...
do.pre.aggregation.processing <- function(append.attributes,
    ...
    else if (outcome.info[['metadata']][['scale']] %in% c('rate', 'time', 'proportion', 'ratio') && !mapping.to.apply$is.identity.mapping) {
...
do.conversion.to.variance <- function(metric,
    ...
    if (metric == "coefficient.of.variance") {
        ...
        dimnames.in.common = get.dimension.values.overlap(dimnames(data.to.convert), dimnames(estimate.data.for.cv))
        data.to.convert = array.access(data.to.convert, dimnames.in.common)
        estimate.data.for.cv = array.access(estimate.data.for.cv, dimnames.in.common)
        
        # sd = cv * mean
        data.to.convert = data.to.convert * estimate.data.for.cv
    }
    
    # Convert to variance
    if (metric %in% c("coefficient.of.variance", "standard.deviation"))
        data.to.convert = data.to.convert ** 2
    
    # Return both data.to.convert and estimate.data.for.cv, which will be needed later for re-conversion
    list(converted.data=data.to.convert, estimate.data.for.cv=estimate.data.for.cv)
}
...
do.conversion.from.variance <- function(metric,
    ...
    if (metric %in% c("standard.deviation", "coefficient.of.variance")) data.to.convert <- sqrt(data.to.convert)
    ...
    if (metric == "coefficient.of.variance") {
        ...
        if (!dim.names.equal(dimnames(data.to.convert), dimnames(estimate.data.for.cv))) {
            dimnames.in.common = get.dimension.values.overlap(dimnames(data.to.convert), dimnames(estimate.data.for.cv))
            data.to.convert = array.access(data.to.convert, dimnames.in.common)
            estimate.data.for.cv = array.access(estimate.data.for.cv, dimnames.in.common)
        }
...
do.mapping.fraction <- function(data.to.convert,
                                mapping.to.apply,
                                dimnames.for.apply,
                                denominator.outcome,
                                source.name,
    ...
    denom.to.data.mapping = get.ontology.mapping(dimnames(denominator.array), as.ontology(dimnames(data.to.convert), incomplete.dimensions=c('year', 'location'))) # made it as.ontology b/c couldn't map year otherwise
    ...
    if (!setequal(dimnames(denominator.array)$year, dimnames(data.to.convert)$year) || !setequal(dimnames(denominator.array)$location, dimnames(data.to.convert)$location))
        data.to.convert = array.access(data.to.convert, year=dimnames(denominator.array)$year, location=dimnames(denominator.array)$location)
