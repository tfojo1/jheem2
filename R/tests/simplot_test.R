...
if (!exists('surveillance.manager'))
    surveillance.manager = load.data.manager('../jheem_analyses/cached/surveillance.manager_112023.rdata')
...
my.simplot = simplot(sim=make.dummy.sim(version='ehe',
                                        ...
                                        from.year=2006,
                                        to.year=2023),
                     outcomes=c('population'), # remember to add in the hard-coded corresponding observed outcomes in PLOTS_simplot.R
                     split.by = 'risk',
                     facet.by = c('sex'),
