...
test.footer <- function (func_name) {
    if (total.count == passed.count) {
        cat(cyan(sprintf("-- %s : All Tests Passed: %g/%d --\n",func_name,passed.count,total.count)))
    } else {
        cat(red(sprintf("-- %s : Some Tests Passed: %g/%d --\n",func_name,passed.count,total.count)))
    }
}
...
test.footer("get.years.for.year.value")
